...
- Warm flow: ~8–10 min, 2–3 passes through the sequence, deepening range each pass.
- Blocks: 3 blocks × ~10 min default for a 45 min class; 2 longer blocks for a narrower focus.
- Work is slow and controlled — positions held or owned for breaths/reps, not raced against a clock.
...
- Off-skate skating-focused drills (ankle stability, single-leg control) are welcome here — they're mobility/balance work without skates.

## Output

A flowing sequence sheet:

1. Warm flow: the sequence in order, transition cues.
2. Each block: area, exercises in order, breath/rep prescription, one coaching cue each.
3. Close: brief down-regulation / stillness note.
